...
export const GENESIS_ACCOUNT =
  GENESIS_ACCOUNTS[process.env.SOROBAN_NETWORK_LOCAL] ??
  GENESIS_ACCOUNTS.standalone;
...
export const loanManagerAddress = (): string =>
  process.env.CONTRACT_ID_LOAN_MANAGER ||
  readTextFile("./.stellar/contract-ids/loan_manager.txt");
...
const bind = (contractName: string, address: string | undefined) => {
  const address_ =
    address || readTextFile(`./.stellar/contract-ids/${contractName}.txt`);
  exe(
    `stellar contract bindings typescript --contract-id ${address_} --network local --output-dir ./packages/${contractName} --overwrite`,
  );
...
const importContract = (contractName: string) => {
  ...
  const importContent =
    ...
    `export const contractId = Client.networks.standalone.contractId; \n\n` +
