...
const setupTrustlineAndFund = async (recipient: Keypair, issuer: Keypair, tickers: string[], amounts: string[]) => {
  ...
  const trustTx = new TransactionBuilder(recipientAccount, { fee: BASE_FEE, networkPassphrase: NETWORK }).setTimeout(
    30,
  );
  ...
  const sendTx = new TransactionBuilder(issuerAccount, { fee: BASE_FEE, networkPassphrase: NETWORK }).setTimeout(30);
  for (let i = 0; i < tickers.length; i++) {
    sendTx.addOperation(
      Operation.payment({
        destination: recipient.publicKey(),
        asset: new Asset(tickers[i]!, issuer.publicKey()),
        amount: amounts[i]!,
      }),
    );
  }
...
export const seedPools = async (
  ...
  exe(
    `stellar contract invoke --id ${xlmPoolId} --source-account ${account} --network testnet -- deposit --user ${initKeypair.publicKey()} --amount ${xlmAmount}`,
  );

  console.log(`Seeding USDC pool with ${usdcAmount / SCALAR_7} USDC...`);
  exe(
    `stellar contract invoke --id ${usdcPoolId} --source-account ${account} --network testnet -- deposit --user ${initKeypair.publicKey()} --amount ${usdcAmount}`,
  );

  console.log(`Seeding EURC pool with ${eurcAmount / SCALAR_7} EURC...`);
  exe(
    `stellar contract invoke --id ${eurcPoolId} --source-account ${account} --network testnet -- deposit --user ${initKeypair.publicKey()} --amount ${eurcAmount}`,
  );
};
...
export const fundFaucet = async (tokens: IssuedTokens, faucetAddress: string) => {
  ...
  exe(
    `stellar contract invoke --id ${tokens.usdcAddress} --source-account ${issuer.secret()} --network testnet -- transfer --from ${issuer.publicKey()} --to ${faucetAddress} --amount ${amount}`,
  );
  exe(
    `stellar contract invoke --id ${tokens.eurcAddress} --source-account ${issuer.secret()} --network testnet -- transfer --from ${issuer.publicKey()} --to ${faucetAddress} --amount ${amount}`,
  );
  exe(
    `stellar contract invoke --id ${tokens.laiAddress} --source-account ${issuer.secret()} --network testnet -- transfer --from ${issuer.publicKey()} --to ${faucetAddress} --amount ${amount}`,
  );
};
